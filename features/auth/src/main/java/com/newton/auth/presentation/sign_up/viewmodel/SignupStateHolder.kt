...
class SignupStateHolder @Inject constructor(
    ...
    private fun confirmPasswordChanged(confirmPassword: String) {
        val validateResult = validator.validatePasswordMatch(
    ...
    fun isFormValid(): Boolean {
        ...
        val confirmPasswordValidation = validator.validatePasswordMatch(
            _signUpState.value.passwordInput,
    ...
    fun setOtpServerError(message: String?) {
        _signUpState.update { it.copy(otpServerError = message, otpError = if (message != null) "Invalid OTP" else null) }
    }

    fun setSuccess(message: String?, authFlow: AuthFlow) {
        _signUpState.update { it.copy(
            isLoading = false,
            errorMessage = null,
            success = message,
